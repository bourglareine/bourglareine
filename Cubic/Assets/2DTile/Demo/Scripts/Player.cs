...
public class Player : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
		;
        if (!IsGrounded) {
			hit = Physics2D.Raycast (new Vector2 (transform.position.x, transform.position.y), -Vector2.up,  gameObject.GetComponent<BoxCollider2D>().size.y / 2.0f,LayerMask.NameToLayer("Player"));
