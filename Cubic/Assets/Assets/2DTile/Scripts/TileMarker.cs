...
public class TileMarker : MonoBehaviour
{
	...
	public void UpdateMarker(Material _mat,Vector2 _tileSize,Vector2 _offset)
	{
		...
		float a = GetComponent<Renderer>().sharedMaterial.mainTexture.width / 32;
		float offset = 1 / a;
		GetComponent<Renderer>().sharedMaterial.mainTextureScale = new Vector2(offset * _tileSize.x, offset * _tileSize.y);
		
		float tO1 = 1 - (offset * (_offset.y + 1));
		float tO2 = -offset + offset * (_tileSize.y);
		GetComponent<Renderer>().sharedMaterial.mainTextureOffset = new Vector2(offset * _offset.x, tO1 - tO2);
